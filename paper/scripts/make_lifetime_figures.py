...
def fig_combined(df: pd.DataFrame, seed: int, out: Path) -> None:
    ...
    fig, (ax, bx) = plt.subplots(2, 1, figsize=(3.45, 2.5), dpi=400, sharex=True,
                                 gridspec_kw=dict(height_ratios=[1.15, 1.0], hspace=0.07))
    FS, FT, FL = 6.5, 6.0, 5.6   # axis labels, ticks/annotations, legend
    # (a) perplexity
    for pol in ["random", "sequential", "hardware_only", "static_sensitivity"]:
        g = d[d.policy == pol].groupby("t").nll
        mean = np.exp(g.mean().reindex(ts).values)
        ...
        ax.fill_between(ts, lo, hi, color=st["color"], alpha=0.18, lw=0)
        ax.plot(ts, mean, color=st["color"], marker=st["marker"], lw=lw, ms=3.6, label=st["label"], zorder=3)
    ax.axhline(DIAG_PPL, color="#555555", ls="-.", lw=0.8)
    ax.text(ts[0] + 0.5, DIAG_PPL - 0.3, f"HWA unclipped digital {DIAG_PPL:.2f} (diagnostic)",
            fontsize=FT, color="#555555", va="top")
    ax.axhline(NOMINAL_PPL, color="#1f5fa8", ls=":", lw=0.9)
    ax.text(ts[-1], NOMINAL_PPL + 0.3, f"nominal all-analog {NOMINAL_PPL:.2f}", fontsize=FT,
            color="#1f5fa8", ha="right", va="bottom")
    ax.axhline(PRETRAINED_PPL, color="#111111", ls="--", lw=0.9)
    ax.text(ts[-1], PRETRAINED_PPL + 0.3, "pretrained digital 29.0 (deployment alternative)",
            fontsize=FT, color="#111111", ha="right", va="bottom")
    band_patch = plt.Rectangle((0, 0), 1, 1, color="#bbbbbb", alpha=0.5)
    handles, labels = ax.get_legend_handles_labels()
    ax.legend(handles + [band_patch], labels + ["min–max range (n = 3)"], loc="upper left",
              fontsize=FL, ncol=2, frameon=True, framealpha=0.95, columnspacing=0.8,
              handlelength=1.6, handletextpad=0.5, borderpad=0.4, labelspacing=0.3)
    ax.set_ylabel("held-out test perplexity", fontsize=FS, labelpad=2)
    ax.set_ylim(28.0, 50.0)
    ax.grid(color="#e5e5e5", lw=0.4)
    ax.text(0.985, 0.97, "(a)", transform=ax.transAxes, fontsize=7, fontweight="bold", va="top", ha="right")
    ...
    for pol, emph in [("sequential", False), ("random", False), ("hardware_only", True)]:
        ...
        bx.plot(ts, mean, color=st["color"], marker=st["marker"], lw=1.7 if emph else 0.9,
                ms=4.0 if emph else 3.3, alpha=1.0 if emph else 0.85, label=f"vs {pol}",
                zorder=4 if emph else 3)
        bx.text(ts[-1] + 2.0, mean[-1], f"vs {pol}", fontsize=FT if emph else FT - 0.5,
                fontweight="bold" if emph else "normal", color=st["color"], va="center")
    bx.axhline(0.0, color="#333333", lw=0.7)
    handles, labels = bx.get_legend_handles_labels()
    order = [labels.index("vs hardware_only"), labels.index("vs random"), labels.index("vs sequential")]
    bx.legend([handles[i] for i in order] + [band_patch],
              [labels[i] for i in order] + ["min–max range (n = 3)"],
              loc="upper left", fontsize=FL, frameon=True, framealpha=0.95,
              handlelength=1.6, handletextpad=0.5, borderpad=0.4, labelspacing=0.3)
    bx.set_xticks(ts)
    bx.set_xlim(ts[0] - 3, ts[-1] + 52)
    bx.set_xlabel("trace timestep (hardware degradation →)", fontsize=FS, labelpad=2)
    bx.set_ylabel("paired ΔNLL improvement (nats)", fontsize=FS, labelpad=2)
    bx.set_ylim(-0.008, 0.165)
    bx.grid(color="#e5e5e5", lw=0.4)
    bx.text(0.985, 0.10, "(b)", transform=bx.transAxes, fontsize=7, fontweight="bold", va="bottom", ha="right")
    for a in (ax, bx):
        for s in ("top", "right"):
    ...
    fig.subplots_adjust(left=0.15, right=0.99, top=0.975, bottom=0.13, hspace=0.07)
